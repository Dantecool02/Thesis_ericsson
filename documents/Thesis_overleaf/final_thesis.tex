...
\begin{table}[!ht]
\centering
\caption{Routing-label classification performance on the held-out test split. \textbf{Bal.\ rt.-lbl acc.}\ is balanced routing-label accuracy, the average of recall on \texttt{mix\_required} and recall on \texttt{naive\_enough}; it corrects for the class imbalance. \textbf{Rt.-lbl acc.}\ is the unweighted fraction of test queries on which the policy agrees with the judge-derived routing label. \textbf{P\textsubscript{mix}}, \textbf{R\textsubscript{mix}}, and \textbf{F1\textsubscript{mix}} are precision, recall, and F1 on the \texttt{mix\_required} class at the validation-selected threshold. All accuracy values are routing-label accuracies and are distinct from final answer accuracy. For the constant-score static baselines, \gls{auroc} equals $0.500$ and \gls{auprc} equals the test-set \texttt{mix\_required} base rate of $0.254$. The type-only diagnostic baseline is non-deployable because it uses 2WikiMultihopQA metadata rather than only query text. For TF-IDF, bracketed values are $95\,\%$ bootstrap intervals over $1\,000$ test-set resamples. For ModernBERT, values are reported as mean $\pm$ standard deviation across three training seeds.}
