...
The distinction studied here is operational rather than causal. A query labeled \texttt{mix\_required} means that \emph{Mix} was judged correct when \emph{Naive} was not under the chosen LightRAG configuration and judge prompt. It does not prove that graph traversal alone caused the improvement, because \emph{Mix} also changes context assembly and retrieval breadth.
...
\begin{itemize}
    ...
    \item How much of the routing signal is explained by 2WikiMultihopQA question type, and how much remains predictable from query text alone?
\end{itemize}
...
\item Evaluate the routers both as held-out classifiers, using routing-label metrics such as \gls{auprc} and F1 on the \emph{Mix}-beneficial class, and as thresholded offline routing policies measured against always-\emph{Naive} and always-\emph{Mix} baselines on mean routed execution time and estimated mean routed model-token usage.
\end{enumerate}

\section{Research methodology}
\label{sec:researchMethodology}

This thesis is a quantitative empirical study on a public multi-hop question-answering benchmark, as is common for evaluating retrieval and routing methods. The predictability of retrieval-mode selection from the query text is evaluated by training query-only routers on judge-derived routing labels and obtaining classification metrics on held-out data. These include \gls{auprc}, balanced routing-label accuracy, and F1 on the minority class. The same routers are additionally evaluated as thresholded offline routing policies and compared against static baseline policies along routed-cost metrics. Bootstrap confidence intervals are used to estimate the uncertainty of these metrics. Comparisons between the learned routers and the static baselines are then used to answer the research question.

The reported classification metrics measure agreement with the judge-derived routing label, not direct final-answer accuracy of the selected retrieval path. In particular, routing a \emph{Naive}-sufficient query to \emph{Mix} is counted as a routing-label error because the cheaper mode was sufficient, even though the \emph{Mix} answer may still be correct. This distinction is important for interpreting the static always-\emph{Mix} baseline and the routed-cost results in Chapter~\ref{ch:implementation}.
...
The offline policy evaluation does not re-judge newly generated answers, because it reuses precomputed \emph{Naive} and \emph{Mix} outputs. Unless stated otherwise, reported accuracy values refer to routing-label accuracy. A false-positive \emph{Mix} decision is therefore counted as a routing-label error because \emph{Naive} was judged sufficient, even though the \emph{Mix} answer may still be correct. Conversely, a false-negative \emph{Mix} decision is an under-routing error, because the router selects \emph{Naive} on a query where \emph{Mix} was judged necessary under the labeling protocol.
...
Hu et al.\ \cite{hu2024routerbench} formalize routing between language models as a cost and quality trade-off, and provide RouterBench as a benchmark for it. Although RouterBench routes between language models rather than between retrieval modes, the framing carries over directly. The trade-off plots used in Chapter~\ref{ch:implementation} follow the same logic. Each operating point of the learned router can be compared to static baselines, and the relevant question is whether the router achieves useful routing-label performance at substantially lower routed cost than the always-expensive policy.

A difference between model-routing benchmarks and the setting studied here is that a wrong expensive-route decision does not necessarily reduce answer quality. If a query is \emph{Naive}-sufficient, routing it to \emph{Mix} is a cost error rather than necessarily an answer error. This makes retrieval-mode routing slightly different from ordinary binary classification. False negatives risk answer failure by routing a \emph{Mix}-beneficial query to \emph{Naive}, while false positives mainly waste computation by routing a \emph{Naive}-sufficient query to \emph{Mix}. The evaluation in this thesis therefore reports routing-label metrics separately from routed cost.
...
The label name \texttt{mix\_required} is retained as a compact code label, but it should be interpreted operationally as \emph{Mix}-beneficial under this judged LightRAG setup. It does not prove that graph traversal alone caused the improvement, because \emph{Mix} also changes context assembly, retrieval breadth, and the evidence exposed to the generator.
...
\begin{table}[!ht]
\begin{center}
\caption{Label-flow summary from raw benchmark samples to the final binary routing dataset. The binary router is trained only on \texttt{naive\_enough} and \texttt{mix\_required} rows. The \texttt{mix\_required} label is operational and means \emph{Mix}-beneficial under the judged LightRAG setup.}
\label{tab:datasetFlow}
\begin{tabular}{l|r}
\textbf{Stage} & \textbf{Count} \\
\hline
Sampled 2WikiMultihopQA queryable samples & 2\,279 \\
...
Evaluation separates routing-label prediction from routed-cost analysis. Held-out classification metrics measure agreement with the judge-derived routing label. They do not directly measure final answer accuracy of the selected retrieval path. In particular, routing a \texttt{naive\_enough} query to \emph{Mix} is counted as a routing-label error because the cheaper mode was sufficient, but the \emph{Mix} answer may still be correct. Conversely, routing a \texttt{mix\_required} query to \emph{Naive} is both a routing-label error and an answer-risk error under the label definition.
...
\end{cases}
\end{equation}
where \(M\) denotes \emph{Mix} and \(N\) denotes \emph{Naive}. At a chosen threshold the reported classification metrics are routing-label accuracy, balanced routing-label accuracy, precision and recall on the \texttt{mix\_required} class, and the F1 score
\begin{equation}
\label{eq:f1}
F_1 = 2 \cdot
...
\end{equation}
The word accuracy is therefore used in the routing-label sense throughout this thesis. It measures whether the policy selected the same mode as the judge-derived binary label and should not be read as final answer accuracy of the routed QA system. Balanced routing-label accuracy is included because the class distribution is imbalanced and plain routing-label accuracy can be high even for a trivial always-\emph{Naive} policy.
...
\right],
\end{equation}
where \(m\) is the number of held-out queries. The mean routed model-token cost is computed analogously, using the per-mode means $\bar{u}^N$ and $\bar{u}^M$ from the instrumented subset (Section~\ref{sec:methodDualMode}) in place of the per-query times:
\begin{equation}
...
The same simulation is run for two static baseline policies for comparison. The first is an always-\emph{Naive} policy, and the second is an always-\emph{Mix} policy. These baselines bracket what a static retrieval strategy can achieve on the test set without any per-query decision and serve as the reference points against which the learned router's routing-label and routed-cost trade-off is reported.
...
\end{equation}
The over-routing rate is the fraction of test queries where the true label is \texttt{naive\_enough} but the policy routes to \emph{Mix},
\begin{equation}
...
\end{equation}
Under-routing is the error type most directly associated with answer-quality risk, because the policy selected \emph{Naive} on a query where \emph{Mix} was judged necessary. Over-routing is primarily a cost error, because the policy selected \emph{Mix} on a query where \emph{Naive} was already judged sufficient. This decomposition is useful because a false-positive \emph{Mix} decision is counted as a routing-label error even though the \emph{Mix} answer may still be correct.
...
A non-deployable question-type diagnostic baseline is also included to measure how much of the routing signal is explained by 2WikiMultihopQA metadata. The rule predicts \emph{Mix} for \texttt{compositional} questions and \emph{Naive} for all other question types, because \texttt{compositional} is the only question type whose \emph{Mix}-beneficial rate exceeds $50\,\%$ in the training split. This baseline is not a realistic deployment policy because question-type metadata is unavailable for arbitrary user queries. It is included only as a diagnostic reference for benchmark structure and as a way to separate how much of the routing signal comes from this coarse metadata rule from how much the learned routers add on top of it.
...
Tables~\ref{tab:classificationResults} and \ref{tab:routedCostResults} summarize the test-set results. Table~\ref{tab:classificationResults} reports routing-label classification performance, while Table~\ref{tab:routedCostResults} reports offline routed cost and policy diagnostics. The separation is important because a false-positive \emph{Mix} decision is a routing-label error but not necessarily an answer error. The policy pays for \emph{Mix} even though \emph{Naive} was already judged sufficient. A false-negative \emph{Mix} decision is more serious, because the policy sends a \emph{Mix}-beneficial query to \emph{Naive}.

ModernBERT achieves higher point estimates than \gls{tfidf} on aggregate routing-label metrics, including \gls{auprc}, balanced routing-label accuracy, F1 on the \emph{Mix}-beneficial class, and routing-label accuracy. The gain comes mainly from higher precision. ModernBERT sends fewer \emph{Naive}-sufficient queries to \emph{Mix}, so its over-routing rate is lower ($0.078 \pm 0.019$ vs.\ $0.155$ for \gls{tfidf}). \gls{tfidf} is more conservative on the routing decision in the opposite direction, with higher \emph{Mix} recall and therefore fewer under-routed \emph{Mix}-beneficial queries ($0.057$ vs.\ $0.076 \pm 0.009$ for ModernBERT), but it also routes more unnecessary queries to \emph{Mix}. This difference matters for deployment. Under-routing is the error type most directly associated with answer-quality risk, while over-routing mainly increases cost. ModernBERT therefore behaves as a more selective lower-cost policy, whereas \gls{tfidf} behaves as a more conservative policy that spends more tokens to reduce missed \emph{Mix}-beneficial queries.

The type-only diagnostic baseline is surprisingly strong, confirming that benchmark question type explains part of the routing signal. However, this baseline uses metadata that is not available in real deployments. Its role is therefore diagnostic rather than competitive. ModernBERT achieves higher point estimates than this metadata rule on F1 ($0.699$ vs.\ $0.667$), precision ($0.701$ vs.\ $0.585$), and routing-label accuracy ($0.846$ vs.\ $0.803$) while using only the query text. \gls{tfidf} matches the diagnostic baseline's \emph{Mix} recall ($0.776$) at lower precision and overall accuracy. The cost picture is also clearer when the two cost dimensions are separated. \gls{tfidf} routes at $8\,531$ mean tokens per query and ModernBERT at $7\,138$ mean tokens per query, compared with $17\,857$ tokens for always-\emph{Mix}, a token saving of $52\,\%$ for \gls{tfidf} and $60\,\%$ for ModernBERT relative to the expensive baseline.
...
\caption{Routing-label classification performance on the held-out test split. \texttt{P\textsubscript{mix}}, \texttt{R\textsubscript{mix}}, and \texttt{F1\textsubscript{mix}} denote precision, recall, and F1 on the \texttt{mix\_required} class. Accuracy values are routing-label accuracies, not final answer accuracies. For the constant-score static baselines, \gls{auroc} equals $0.500$ and \gls{auprc} equals the test-set \texttt{mix\_required} base rate of $0.254$. The type-only diagnostic baseline is non-deployable because it uses 2WikiMultihopQA metadata rather than only query text. For TF-IDF, bracketed values are $95\,\%$ bootstrap intervals over $1\,000$ test-set resamples. For ModernBERT, values are reported as mean $\pm$ standard deviation across three training seeds.}
...
\caption{Offline routed-cost and policy diagnostics on the held-out test split. Mean tokens are estimated from per-mode token means measured on the instrumented \(N = 150\) query subset (Section~\ref{sec:methodDualMode}). The token saving column reports the reduction in mean tokens relative to the always-\emph{Mix} baseline. Under-routing means routing a \texttt{mix\_required} query to \emph{Naive}. Over-routing means routing a \texttt{naive\_enough} query to \emph{Mix}. Under-routing is the error type most directly associated with answer-quality risk, while over-routing is primarily a cost error. ModernBERT values are the mean over three training seeds $\pm$ standard deviation.}
...
Figure~\ref{fig:costFrontier} combines the two preceding views into a routing-label-versus-routed-time frontier. Each router traces an operating curve in (mean routed time, F1\textsubscript{mix}) space as $\tau$ sweeps. The static always-\emph{Naive} and always-\emph{Mix} policies appear as the two extreme operating points. The learned routers achieve higher routing-label F1 than either static policy at substantially lower mean routed time than always-\emph{Mix}. The ModernBERT router lies slightly above and to the right of the \gls{tfidf} baseline, reaching higher F1\textsubscript{mix} at a slightly higher mean routed time, with the type-only diagnostic baseline placed nearby for reference.
...
Table~\ref{tab:perQuestionType} reports a per-question-type breakdown on the test split. The 2WikiMultihopQA question types have very different routing-label distributions. The \texttt{comparison} slice is almost always \texttt{naive\_enough}, with only $2$ of $90$ test rows judged \emph{Mix}-beneficial. The \texttt{bridge\_comparison} slice sits around one quarter. The \texttt{compositional} slice is the one in which the routing decision matters most, with $38$ of $65$ test rows judged \emph{Mix}-beneficial. The \texttt{inference} slice contributes only $5$ test rows, of which one is \texttt{mix\_required}, so its per-class metrics are reported for completeness but should be read as a description of these particular test rows rather than as a stable estimate. On the \texttt{compositional} slice ModernBERT achieves higher point estimates than \gls{tfidf} on \gls{auprc} (mean $0.839$ vs.\ $0.814$) and F1 on \texttt{mix\_required} (mean $0.799$ vs.\ $0.753$). On \texttt{bridge\_comparison} the picture is more mixed. \gls{tfidf} achieves a slightly higher \gls{auprc} and F1 here ($0.418$ and $0.316$) by routing more queries to \emph{Mix} on this slice, while ModernBERT is more conservative, with lower recall ($0.167$ vs.\ $0.375$) but higher precision ($0.345$ vs.\ $0.273$) and higher overall slice accuracy ($0.707$ vs.\ $0.606$).
...
The headline result is that the retrieval-mode distinction between \emph{Naive} and \emph{Mix} is partly predictable from the query text alone. Both routers outperform the always-\emph{Naive} baseline on the minority class and reach operating points that no static policy at comparable cost can reach in Figure~\ref{fig:costFrontier}. The ModernBERT router achieves higher point estimates than the \gls{tfidf} baseline on \gls{auprc}, balanced routing-label accuracy, F1 on the minority class, and routing-label accuracy, with differences that are stable across three training seeds.

\subsection*{Routing errors: answer risk versus cost waste}

The two routing error types have different practical meanings. A false negative routes a \emph{Mix}-beneficial query to \emph{Naive} and is therefore the error type most directly associated with answer-quality risk. A false positive routes a \emph{Naive}-sufficient query to \emph{Mix} and is primarily a cost error, since the cheaper mode was already judged sufficient. This distinction explains why always-\emph{Mix} has poor routing-label accuracy but is not necessarily poor in final answer quality. It also explains the difference between \gls{tfidf} and ModernBERT in Table~\ref{tab:routedCostResults}. \gls{tfidf} has higher recall and a lower under-routing rate, and therefore fewer under-routed \emph{Mix}-beneficial queries, while ModernBERT has higher precision and a lower over-routing rate, and therefore fewer unnecessary \emph{Mix} calls.

Routing-label performance and routed cost should therefore be interpreted together. A router with high recall behaves conservatively and protects against missed \emph{Mix}-beneficial queries, but may spend more. A router with high precision behaves selectively and saves more cost, but may accept more under-routing risk. The selected ModernBERT operating point is closer to the selective policy, while the \gls{tfidf} operating point is closer to the conservative policy. Neither extreme is universally correct, and which one is preferable depends on the relative cost of an unnecessary \emph{Mix} call versus an answer-quality regression in a given deployment.

\subsection*{Per-question-type behavior and question-type structure}

The most informative slice is \texttt{compositional}. This is where the class distribution is balanced and where \emph{Mix} is judged beneficial most often. On this slice ModernBERT achieves a mean \gls{auprc} of $0.839$ against \gls{tfidf}'s $0.814$ and lifts F1 on \texttt{mix\_required} from $0.753$ to $0.799$, despite the small training set, which suggests that the encoder picks up on more than the lexical patterns a \gls{tfidf} classifier can exploit. The \texttt{bridge\_comparison} slice is harder for both routers. \gls{tfidf} reaches a slightly higher \gls{auprc} and F1 here ($0.418$ and $0.316$) by routing more queries to \emph{Mix} on this slice, while ModernBERT is more conservative on this slice and ends up with higher slice accuracy ($0.707$ vs.\ $0.606$) and higher precision but lower minority recall. Neither outcome is satisfactory, and improving routing on this slice is an obvious target for further work.

The diagnostic question-type baseline confirms that part of the routing signal is explained by benchmark structure. Predicting \emph{Mix} for \texttt{compositional} questions and \emph{Naive} for the rest is already competitive on Table~\ref{tab:classificationResults}, because \texttt{compositional} questions dominate the \emph{Mix}-beneficial class on this benchmark. This does not make the learned router unnecessary, because the question-type label is benchmark metadata and is not available in real user deployments. The result instead shows that ModernBERT learns a deployable approximation of this structure from query text alone and achieves higher point estimates than the coarse metadata rule on F1, precision, and routing-label accuracy.

This affects how the results should be interpreted. The router is not learning a universal law of graph necessity. It is learning patterns in how questions are phrased that correlate with whether \emph{Mix} is judged beneficial under this LightRAG setup and benchmark distribution. The type-only diagnostic baseline is therefore best read as an upper bound on what coarse benchmark structure alone can deliver. Anything ModernBERT adds beyond that bound is a credit to the encoder, but anything it does not exceed is more a description of the benchmark than of any deeper property of the queries.

\subsection*{Benchmark slices versus real deployments}

The per-question-type breakdown and the type-only diagnostic baseline both rely on 2WikiMultihopQA's question-type metadata. In a real deployment that metadata does not exist. Users send free-form queries, and the routing layer sees only the query text. The benchmark slices are useful here as a diagnostic device. They give a way to ask which subpopulations the router is learning to handle well and which it is not, and they help separate signal that comes from coarse structural cues (such as whether the question is a compositional multi-hop lookup) from signal that comes from finer phrasing. They are not a deployment recipe. The point of the learned routers is that they can approximate this structure from the query text alone, which is the only input available outside the benchmark.

This framing is also useful for interpreting the \texttt{compositional}-versus-\texttt{bridge\_comparison} contrast. The fact that ModernBERT is strong on \texttt{compositional} and weaker on \texttt{bridge\_comparison} tells us something about which phrasings the encoder has learned to recognize. It does not tell us that compositional queries are intrinsically easier in production, only that within this benchmark there are more of them and they share a more consistent surface form.

\subsection*{Routing-label accuracy versus final answer accuracy}

This thesis does not report independently re-judged final answer accuracy for each routed policy. The offline simulation selects between precomputed \emph{Naive} and \emph{Mix} outputs, but the primary stored supervision is the routing label. Routing-label accuracy should therefore not be read as final answer accuracy. In particular, always-\emph{Mix} is label-incorrect on \emph{Naive}-sufficient rows because it uses more retrieval than necessary, not because its answer is necessarily wrong. Direct policy answer accuracy would require retained per-mode correctness indicators for every query, or an additional judging pass over the answer selected by each policy.

The reported metrics are still useful for adaptive routing. Routing-label F1 measures whether the router selects the minimum judged sufficient mode. Mean routed time and estimated token usage measure the cost of doing so, and the under-routing and over-routing rates further separate answer-risk errors from cost-waste errors. Treated together, these metrics describe how often the router agrees with the judge and how expensive its agreement is.

\subsection*{Statistical caveats}

There are limitations to the headline result. The held-out test set has $193$ queries, which is enough to compute meaningful bootstrap intervals but not enough to resolve small differences between the routers. \gls{tfidf}'s bootstrap \gls{auroc} interval ($[0.836, 0.936]$) contains the ModernBERT mean \gls{auroc} of $0.908$, so the \gls{auroc} comparison cannot be called significant on this test set. The \gls{auprc} and balanced-routing-label-accuracy gaps are larger but still inside the bootstrap intervals, which is what should be expected on a $193$-row test set. The multi-seed standard deviation for ModernBERT is small on the aggregate routing-label metrics ($\leq 0.017$ on \gls{auprc}, \gls{auroc}, balanced routing-label accuracy, F1, and routing-label accuracy), so the gains are at least stable across seeds. The per-class precision and recall vary more across seeds (standard deviations of $0.057$ and $0.042$ respectively), which is consistent with each seed selecting a slightly different threshold on a $193$-row test set. A larger benchmark would be needed before stronger claims can be made.

\subsection*{Cost picture in time versus tokens}

The cost picture is more pronounced in model tokens than in execution time. The two static baselines bracket a mean routed range of $11.0$ to $24.4$ seconds per query in execution time and roughly $3{,}500$ to $17{,}900$ tokens per query in model-token usage. The learned routers select operating points around $15$ to $17$ seconds (a $30$--$39\,\%$ reduction in time relative to always-\emph{Mix}) and $7{,}100$ to $8{,}500$ tokens (a $52$--$60\,\%$ reduction in tokens relative to always-\emph{Mix}), at non-trivial \texttt{mix\_required} F1. The token saving is more dramatic than the time saving because \emph{Mix} adds one extra LLM call per query and a substantially larger assembled context \cite{lightrag2024,microsoft2024graphragcosts}, whereas time is partly absorbed by network latency that is independent of token volume. The routing decision itself contributes essentially zero LLM tokens, since neither router calls a generative model. It contributes only milliseconds of compute time, which is negligible next to retrieval cost on either dimension.
...
This thesis studied whether the choice between LightRAG's cheap \emph{Naive} retrieval mode and its more expensive \emph{Mix} retrieval mode can be predicted from the query text alone, on the 2WikiMultihopQA benchmark, using a lightweight encoder-only classifier rather than a large language-model controller. Routing labels were derived by executing both retrieval modes on each benchmark question and using an \gls{llmjudge} \cite{zheng2023judge} to compare each mode's answer against the gold answer. The label \texttt{mix\_required} is therefore operational. It means \emph{Mix} was judged correct when \emph{Naive} was not under the chosen LightRAG configuration and judge prompt, not that graph traversal alone caused the improvement.

On the held-out test split, the ModernBERT router achieves higher point estimates than a \gls{tfidf} logistic-regression baseline on \gls{auprc} ($0.769$ vs.\ $0.742$), balanced routing-label accuracy ($0.798$ vs.\ $0.784$), F1 on the minority \texttt{mix\_required} class ($0.699$ vs.\ $0.650$), and routing-label accuracy ($0.846$ vs.\ $0.788$), while remaining within the bootstrap interval on \gls{auroc}. The multi-seed standard deviation of the ModernBERT router is small on the aggregate routing-label metrics ($\leq 0.017$ on \gls{auprc}, \gls{auroc}, balanced routing-label accuracy, F1, and routing-label accuracy), which makes the differences stable rather than seed-specific. Both learned routers reach test-set operating points around $15$ to $17$ seconds of mean routed execution time, compared with $11.0$ seconds for always-\emph{Naive} and $24.4$ seconds for always-\emph{Mix}, and achieve higher routing-label F1 than either static policy at substantially lower mean routed time than always-\emph{Mix}. Measured in model tokens, the same routers route at roughly $7{,}100$ to $8{,}500$ mean tokens per query, compared with $3{,}458$ for always-\emph{Naive} and $17{,}857$ for always-\emph{Mix}, a routing saving of $52$--$60\,\%$ relative to the expensive baseline. The error decomposition tells a complementary story. ModernBERT trades a slightly higher under-routing rate ($0.076 \pm 0.009$ vs.\ $0.057$ for \gls{tfidf}) for a substantially lower over-routing rate ($0.078 \pm 0.019$ vs.\ $0.155$ for \gls{tfidf}), which is the source of its token saving.

The per-question-type breakdown localizes most of the routing signal in the \texttt{compositional} slice, where \emph{Mix} was judged beneficial on $58\,\%$ of the test rows and where ModernBERT achieves a roughly two-and-a-half-point higher \gls{auprc} than \gls{tfidf}. The \texttt{comparison} slice is essentially decidable as \emph{Naive} and both routers correctly avoid \emph{Mix} on it. The \texttt{bridge\_comparison} slice is the weakest part of the result and is the natural target for further work. The type-only diagnostic baseline shows that a substantial part of this signal is explained by 2WikiMultihopQA's question-type metadata alone. ModernBERT achieves higher point estimates than that metadata rule on F1, precision, and routing-label accuracy while using only the query text, which is the input available in a real deployment.

Returning to the research question, the \emph{Naive}-sufficient versus \emph{Mix}-beneficial distinction is partly predictable from query text alone in this setting. ModernBERT achieves higher point estimates than \gls{tfidf} and static baselines on several routing-label metrics while reducing routed token cost substantially relative to always-\emph{Mix}. These results support learned query routing as a promising cost-control layer for Hybrid \gls{rag}, but the reported accuracy values are routing-label accuracies rather than independently measured final answer accuracies.

The learned router is not best understood as proving that graph retrieval is intrinsically required for particular question types. It predicts an operational label, namely whether LightRAG \emph{Mix} was judged beneficial relative to \emph{Naive} under a fixed benchmark, index, generator, and judge prompt. Within that scope, the result is positive. The router can often identify when the expensive path is unnecessary, and can therefore reduce routed cost without always defaulting to the most expensive retrieval mode.
...
Future work should log token usage for every \emph{Naive} and \emph{Mix} execution in the full $1{,}286$-row routing dataset. This would make it possible to compute per-query routed token cost directly, report uncertainty intervals for token cost, and study whether token usage varies systematically by question type or routing label.

\subsection{Judge labels}
\label{sec:limitJudgeLabels}

Routing labels come from an \gls{llmjudge} and are treated as judged supervision rather than absolute ground truth. Although the judge sees the gold answer and the same prompt is used consistently across the dataset \cite{zheng2023judge}, judge errors may still affect the labels. This is especially relevant for borderline answers, partially correct answers, and answers that use different surface forms from the gold answer.

The label names should also be interpreted operationally. A \texttt{mix\_required} label means that \emph{Mix} was judged correct when \emph{Naive} was not, under the chosen LightRAG configuration and judge prompt. It does not prove that the graph component alone caused the improvement, because \emph{Mix} also changes context assembly, retrieval breadth, and the evidence exposed to the generator.
...
This appendix lists the configuration used to produce the routing labels, train the routers, and compute the reported routed-cost estimates. Table~\ref{tab:lightragDataJudgeConfig} summarizes the LightRAG, data, and judge settings. Table~\ref{tab:routerTrainingConfig} summarizes the router training settings. The exact judge prompt is reproduced in Section~\ref{app:judgePrompt}.

\begin{table}[!ht]
\centering
\caption{LightRAG, data, and judge configuration used in this thesis. The same generator model and embedding model are used during indexing and at query time. All Gemini calls use a temperature of $0.0$.}
...
\begin{table}[!ht]
\centering
\caption{Router training configuration used in this thesis.}
...
The judge receives the question, the gold answer, the \emph{Naive} answer, and the \emph{Mix} answer in a JSON payload, and is asked to return exactly one of the three class labels \texttt{naive\_enough}, \texttt{mix\_required}, or \texttt{none\_enough}. The system prompt is reproduced verbatim below.
...
The label is assigned as follows. \texttt{naive\_enough} means the \emph{Naive} answer matches the gold answer. \texttt{mix\_required} means the \emph{Naive} answer does not match the gold answer but the \emph{Mix} answer does. \texttt{none\_enough} means neither answer matches the gold answer and the row is excluded from the binary routing dataset.
