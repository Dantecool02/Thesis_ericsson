...
Third, this selectivity translates into cost: at its validation-selected threshold the ModernBERT policy reduces mean routed execution time by $42\,\%$ and estimated mean routed model-token usage by $63\,\%$ relative to always-\emph{Mix} (\gls{tfidf}: $30\,\%$ and $52\,\%$) while, under the stated assumption about \emph{Mix} correctness, retaining about $92\,\%$ (\gls{tfidf}: $94\,\%$) of always-\emph{Mix}'s judged-correct answers.
...
Token measurement subset & \(N = 15\) queries spanning all four 2WikiMultihopQA question types \\
...
Run both retrieval modes on every queryable sample & \path{src/data_synthesis/query_lightrag.py} \\
...
Run held-out classification, routed-cost simulation, per-question-type breakdown, and bootstrap confidence intervals for the seed-averaged ensemble & \path{src/router/thesis_analysis_ensemble.py} \\
Paired statistical tests (paired bootstrap, permutation, McNemar, DeLong) and per-seed metrics & \path{src/router/thesis_stats_tests.py} \\
