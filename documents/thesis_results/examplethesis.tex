...
The simplest retrieval layer is vector similarity search over document chunks. It is efficient and often effective for local factual questions, but it can struggle when a query depends on linking evidence across documents, resolving relationships between entities, or comparing distributed attributes. Graph-enhanced \gls{rag} systems address this by introducing structured representations of entities, relations, or higher-level document organization. Recent systems such as GraphRAG and LightRAG show how graph-based structure can support retrieval beyond isolated nearest-neighbor chunks \cite{edge2024graphrag,lightrag2024}.

Richer retrieval is not free, though. Graph construction, graph traversal, larger context assembly, and additional language-model calls can increase execution time. At Ericsson, where internal technical repositories are large and fragmented, graph-enhanced retrieval may improve answer quality for difficult questions, but applying it to every query can waste computation on questions that cheaper vector retrieval already handles \cite{jeong2024adaptiverag,hu2024routerbench}.

This tension motivates adaptive retrieval. Self-RAG and Adaptive-RAG argue that retrieval behavior should depend on the query rather than follow a fixed policy \cite{asai2023selfrag,jeong2024adaptiverag}. More broadly, the model-routing literature frames this as a cost--performance decision, where different execution paths carry different expected benefits and costs, and a router should select the path appropriate for the input \cite{hu2024routerbench}.

The question studied here is narrower than general model routing. Instead of routing between different language models, the project routes between retrieval modes inside a fixed Hybrid \gls{rag} setting. The low-cost path is LightRAG's \emph{Naive} mode, used as a vector-retrieval baseline, and the high-cost path is LightRAG's \emph{Mix} mode, which combines vector retrieval with graph-enhanced retrieval. The routing layer is intentionally lightweight, because if the router itself requires a large decoder model, much of the intended efficiency gain is lost. Prior work on encoder-only classification supports this design for narrow text-classification tasks \cite{benayas2024encoder}.

\section{Problem}
\label{sec:problem}

Hybrid \gls{rag} systems expose a practical tension between retrieval quality and retrieval cost. A cheap vector-based path suffices for many questions, while a graph-enhanced path helps when the answer depends on relationships or evidence distributed across several documents. A static policy cannot exploit this distinction, because always using the cheap path risks missing cases where structured retrieval is needed, while always using the expensive path wastes computation on easier queries.

The project focuses on that decision point. In LightRAG, the choice is operationalized as \emph{Naive} retrieval (the lower-cost vector path) versus \emph{Mix} retrieval (the more expensive graph-enhanced path). Prior work motivates both graph-based \gls{rag} and adaptive retrieval \cite{edge2024graphrag,lightrag2024,asai2023selfrag,jeong2024adaptiverag}, but does not answer whether this particular retrieval-mode choice can be predicted before retrieval, from the query text alone.

\subsection{Original problem and definition}
\label{sec:researchQuestion}

The practical motivation comes from industrial \gls{qa} over large technical repositories. Ericsson works with internal documentation where relevant information is fragmented across documents and absent from public language-model pretraining. \Gls{rag} is needed for grounding, but graph-enhanced retrieval should ideally be reserved for queries where the cheaper vector path is insufficient.

The engineering problem is to decide, for a given query, whether cheap vector retrieval suffices or whether graph-enhanced retrieval is required, and to make that decision before the expensive path runs. A ModernBERT-based query classifier serves this role instead of a large general-purpose \gls{llm} router, keeping the control layer itself cheap \cite{benayas2024encoder,warner2024modernbert}.

\subsection{Scientific and engineering issues}
\label{sec:scientificEngineeringIssues}

The scientific issue is whether retrieval need is predictable at all from the query alone. If the distinction between vector-sufficient and graph-required queries is mainly determined by corpus state after retrieval, a query-only router should not work. If the distinction is partly reflected in how the question is phrased, a supervised classifier may learn a useful decision boundary.

The engineering issue is that 2WikiMultihopQA provides answer supervision but not routing labels directly. The project therefore constructs routing labels by executing both retrieval modes on 2WikiMultihopQA and comparing the outputs against benchmark answers using \gls{llmjudge}, treating the judge as a fallible labeling instrument rather than unquestioned ground truth \cite{ho2020twowiki,zheng2023judge}.
...
For Ericsson, internal technical documentation is large, fragmented, and absent from public language-model pretraining. High-quality grounded \gls{qa} is useful, but indiscriminately applying the most expensive retrieval strategy creates unnecessary latency. A successful router would reduce redundant graph-enhanced retrieval while preserving access to it for queries that genuinely require structured evidence.

On the academic side, the goal is narrower than it might seem. The project does not aim to prove that graph retrieval is universally superior, nor that LightRAG is the best possible \gls{rag} framework. The question is whether the need for graph-enhanced retrieval is predictable enough from the query text that a lightweight classifier can serve as a useful control layer. The contribution is therefore empirical, asking whether the vector-sufficient versus graph-required distinction can be learned in a fixed experimental setting.
...
The study is a quantitative empirical experiment on the public 2WikiMultihopQA benchmark. LightRAG serves as a fixed Hybrid \gls{rag} system; every query is run in both \emph{Naive} and \emph{Mix} retrieval modes. The generated answers are then evaluated against benchmark answers with an \gls{llmjudge} procedure, producing routing labels that indicate whether vector retrieval was sufficient or graph-enhanced retrieval was required.

These labels are used to train lightweight query-only routers, a ModernBERT-based classifier and a \gls{tfidf} logistic-regression baseline. Evaluation on held-out data uses classification metrics such as \gls{auprc}, precision, recall, and F1 to measure how reliably the router predicts the judged routing decision.

To connect classifier performance to system efficiency, the predicted probabilities are also evaluated as routing policies. Different thresholds are compared against static baselines (always \emph{Naive}, always \emph{Mix}) using measured execution times and judged answer outcomes.
...
\Gls{rag} connects a generative language model to external knowledge. The original formulation combines parametric knowledge in a neural language model with non-parametric memory retrieved from a document collection \cite{lewis2020rag}. Later surveys describe \gls{rag} more broadly as a family of systems for grounding \gls{llm} outputs in external documents, databases, or other knowledge sources \cite{gao2023ragsurvey}. For industrial \gls{qa} the motivation is straightforward: the relevant facts are often private, domain-specific, or updated after the language model was trained.

A simplified \gls{rag} process can be written as a retrieval-conditional generation problem, following the retrieval-marginalization view in the original formulation but using notation adapted for this project \cite{lewis2020rag}. Given a query \(q\), a retriever selects a set of documents or chunks \(\mathcal{D}_k(q)\), and a generator produces an answer \(a\) conditioned on the query and the retrieved context.
...
Equation~\ref{eq:ragGeneration} is not meant as an implementation claim about every modern \gls{rag} system, but it captures the dependency that matters here. The generated answer depends on which evidence the retrieval system exposes to the language model. Retrieval is not a passive preprocessing step, it is a central part of the system's behavior.

At Ericsson, internal technical documents cannot be assumed to exist in public model pretraining, so a \gls{rag} system is needed to ground answers in company-controlled knowledge. But the retrieval method must still decide how much structure and computation to use for each query. Grounded \gls{qa} is useful; indiscriminate use of expensive retrieval wastes resources.

\section{Vector Retrieval, Hybrid Retrieval, and Graph-Based RAG}
\label{sec:backgroundHybridGraph}

Most practical \gls{rag} systems start with vector retrieval. Documents are split into chunks, each chunk is embedded into a vector space, and queries are matched to chunks by similarity. The original \gls{rag} model indexes chunked Wikipedia passages and retrieves top-\(k\) passages through maximum inner-product search \cite{lewis2020rag}. Dense Passage Retrieval takes a similar approach, representing questions and passages with dense encoders and retrieving by vector similarity for open-domain \gls{qa} \cite{karpukhin2020dpr}. Vector retrieval is therefore a natural low-cost baseline.
...
The cost side matters just as much. Graph-enhanced systems require additional work for graph construction, traversal, context assembly, and language-model calls. The GraphRAG pipeline, for example, uses an \gls{llm} to build graph structure and generate community-level summaries, and Microsoft's own cost discussion identifies repeated \gls{llm} calls during graph construction as a major cost driver \cite{edge2024graphrag,microsoft2024graphragcosts}. Graph retrieval is therefore not an unconditional replacement for vector retrieval, but an expensive capability that should ideally be used only when it is needed.
...
Adaptive retrieval challenges the assumption that the same retrieval strategy should be used for every input. Self-RAG learns to retrieve, generate, and critique through self-reflection, making retrieval behavior conditional rather than fixed \cite{asai2023selfrag}. Adaptive-RAG studies how question complexity can guide the selection of retrieval-augmented reasoning strategies \cite{jeong2024adaptiverag}. Both support the premise that retrieval should depend on the query.

The routing literature frames this more explicitly as a cost--performance problem. RouterBench studies multi-model routing, where different language models have different expected cost and quality characteristics \cite{hu2024routerbench}. Although RouterBench concerns model routing rather than retrieval-mode routing, the abstraction carries over directly because a router observes an input and chooses among execution paths before the expensive computation happens.

The move made here is to adapt routing from the model layer to the retrieval layer. Instead of asking which language model should answer, the project asks which retrieval mode should supply evidence. The cost-aware intuition is the same as in RouterBench and the query-dependent motivation the same as in Adaptive-RAG, but the action space is narrower \cite{hu2024routerbench,jeong2024adaptiverag}. With the retrieval framework fixed and only two modes available, the scientific question becomes measurable.

An important constraint is that the router is query-only. It sees the text of the query and predicts whether graph-enhanced retrieval is required before any retrieval runs. This is consistent with adaptive-retrieval work that predicts query complexity before selecting a strategy \cite{jeong2024adaptiverag,hu2024routerbench}. It makes the task harder than routing with access to retrieved documents or generated answers, but it preserves the potential efficiency gain. A router that needs to run an expensive retrieval step before deciding would undermine its role as a cheap control layer.

Encoder-only language models are well suited to this kind of narrow classification. \Gls{bert} introduced deep bidirectional transformer representations for language understanding \cite{devlin2019bert}, and later comparisons between encoder-only and decoder-only architectures confirm that encoders remain attractive when the output is a class label and efficiency matters \cite{benayas2024encoder}. ModernBERT is a recent encoder designed specifically for efficient fine-tuning and inference \cite{warner2024modernbert}.

The plan is therefore to compare a ModernBERT-based router with a simpler \gls{tfidf} logistic-regression baseline. The baseline matters because the routing distinction may be partly lexical, since some question forms may simply correlate with graph need. ModernBERT tests whether pretrained semantic representations improve beyond those surface patterns.

\section{Related Work}
\label{sec:relatedWork}

GraphRAG and LightRAG demonstrate that graph-enhanced retrieval can add structure beyond local vector similarity \cite{edge2024graphrag,lightrag2024}. They motivate the high-cost retrieval path, but neither answers the question of \emph{when} graph-enhanced retrieval should be selected over a cheaper vector-based path.

Self-RAG and Adaptive-RAG show that retrieval behavior can be conditional rather than fixed \cite{asai2023selfrag,jeong2024adaptiverag}. The present work follows that idea but applies it to a smaller action space: two retrieval modes in one Hybrid \gls{rag} framework rather than a general strategy selector.

RouterBench frames routing as a systems problem where the decision should account for both expected quality and cost \cite{hu2024routerbench}, and recent encoder-only classification work supports using compact bidirectional models for efficient text classification \cite{devlin2019bert,benayas2024encoder,warner2024modernbert}. What is missing from this literature is a focused study of LightRAG's \emph{Naive}-versus-\emph{Mix} choice as a supervised, query-only routing problem. That is the gap the project addresses.
...
Figure~\ref{fig:methodPipeline} shows the end-to-end pipeline. It starts from a public multi-hop \gls{qa} benchmark whose samples are processed and indexed in LightRAG while preserving the mapping between questions, answers, and supporting evidence \cite{ho2020twowiki,lightrag2024}. Each queryable sample is then executed with both \emph{Naive} and \emph{Mix} retrieval, the resulting answers are judged into routing classes, and the usable binary labels are used to train and evaluate query-only routers \cite{zheng2023judge}.
...
The study uses a quantitative experimental paradigm built around a controlled LightRAG setting where the same benchmark query is run with both \emph{Naive} and \emph{Mix} retrieval. Each query therefore produces two generated answers, a judged routing label, and a router prediction that can be compared against that label, making the routing problem directly observable.

The reasoning is mainly deductive. The starting assumption is that some queries contain enough information to indicate whether vector retrieval is likely sufficient or whether graph-enhanced retrieval is needed. If the router predicts the judged routing labels reliably and creates useful thresholded policies, the results support the assumption. If it performs close to trivial baselines, the results suggest that the retrieval-mode distinction is not predictable enough from query text alone in this setting.

The research question is about measurable system behavior, not about interviews, observations, or organizational practice. Ericsson documentation motivates why the problem matters industrially, but the core evidence comes from reproducible benchmark experiments, prediction metrics, and cost--performance comparisons.

\section{Data Collection}
\label{sec:dataCollection}

The primary dataset is 2WikiMultihopQA \cite{ho2020twowiki}, a public benchmark containing questions, answers, context documents, and supporting evidence. It was designed for multi-hop \gls{qa}, where answering depends on combining information across reasoning steps, the kind of scenario where graph-enhanced retrieval is expected to matter most. Related benchmarks such as HotpotQA and MuSiQue share this motivation \cite{yang2018hotpotqa,trivedi2022musique}. No new human-subject data is collected, since the task is to transform the existing benchmark into a form suitable for retrieval-mode routing.

\subsection{Sampling}

The pipeline processes the benchmark in original sample order, converting each sample into a structured record containing the question, answer, question type, context documents, supporting facts, and evidence metadata. The LightRAG indexer then inserts context documents while tracking which samples are queryable. Preserving this mapping matters because each query result must later be connected back to its benchmark answer and routing label.

A held-out split of judged binary examples is used for router evaluation, stratified by routing label so that both \texttt{naive\_enough} and \texttt{mix\_required} examples appear in train, validation, and test partitions.
...
The class distribution is imbalanced, with more examples where \emph{Naive} retrieval is sufficient than examples where \emph{Mix} is required. The evaluation will therefore report class-sensitive metrics such as \gls{auprc}, precision, recall, F1, and balanced accuracy rather than relying only on overall accuracy \cite{saito2015precision}.

\subsection{Target Population}

The immediate target population is 2WikiMultihopQA-style multi-hop questions over a controlled Wikipedia-derived corpus. The broader population of interest is industrial \gls{rag} questions where some queries can be answered by cheap vector retrieval and others require structured retrieval. Whether results on 2Wiki transfer to Ericsson documentation or to other Hybrid \gls{rag} frameworks is an open question, and the conclusions will be scoped accordingly.
...
Training uses backpropagation and the AdamW optimizer \cite{loshchilov2019decoupled}. Since the binary classes are imbalanced, class weights can be used in the loss so that the minority \texttt{mix\_required} class is not ignored during fine-tuning. The actual routing decision is not made by the classification head alone, but by applying a threshold to \(s_\phi(q_i)\) during the policy evaluation described in Section~\ref{sec:dataAnalysisTechnique}. A \gls{tfidf} logistic-regression model is used as a classical baseline.

\subsection{Hardware/Software to be used}
\label{sec:hardwareSoftware}

The implementation uses Python, LightRAG, PyTorch, Hugging Face Transformers, and scikit-learn. Gemini models handle LightRAG language-model calls, and \texttt{gemini-3-flash-preview} serves as the judge in the current setup. \Gls{gpu} access is useful for transformer fine-tuning, but the pipeline is designed to run incrementally so that data generation, baseline training, and offline evaluation do not have to happen in a single uninterrupted run.

Both retrieval-mode outputs and execution times are logged. Execution time is the practical cost proxy because it is available for both modes throughout the pipeline; token usage is treated as supplementary metadata. A practical risk is that \gls{API} cost and throughput can limit how quickly retrieval outputs and judge labels are produced, so the pipeline supports incremental execution and completed outputs are reused when a run is interrupted or extended.
...
The main construct validity risk is that judge labels may not perfectly represent true retrieval need. Prior work on \gls{llmjudge} shows that language models can be useful evaluators, but also that their judgments need to be treated critically \cite{zheng2023judge}. Constrained labels, grounding the judgment in the benchmark answer, and excluding examples where neither mode succeeds mitigate this, but the labels remain imperfect supervision.

An internal validity strength is that both retrieval modes run within the same framework over the same indexed corpus, which reduces confounding from comparing different \gls{rag} systems. The corresponding risk is that observed differences may partly reflect implementation details of LightRAG's modes rather than graph retrieval in general.

A subtler validity risk comes from the stochastic nature of LightRAG graph construction and \gls{llm}-based generation. Entity extraction, relation extraction, summarization, and answer generation can all vary between runs even when the input documents are identical. If the graph were rebuilt several times, differences in router labels might partly reflect graph-construction variance rather than stable differences between \emph{Naive} and \emph{Mix}. To control for this, the experiment treats the constructed index and logged retrieval outputs as a fixed part of the experimental setup.

\subsection{Reliability of method}
\label{sec:reliabilityOfMethod}

Reliability requires that repeated runs lead to the same routing dataset, the same train/validation/test split, and similar router performance. Deterministic preprocessing, stable sample identifiers, fixed data splits, explicit random seeds, and saved prediction files all support this.

The main stochastic components are graph construction, answer generation, judge labeling, and neural router training. Separating data generation from router evaluation reduces their impact. Once answers and judge labels have been produced, the router trains and evaluates on saved examples rather than on live \gls{API} calls. Fixed splits and random seeds make repeated training runs directly comparable.

Stronger alternatives exist. Human expert annotation could provide more reliable routing labels, repeated judging with multiple models could estimate label uncertainty more directly, and rebuilding the LightRAG graph several times could measure graph-construction variance. These are outside the core scope, but constrained judge labels, exclusion of unclear examples, and held-out evaluation partly compensate.

The key reliability check is whether the router does more than exploit the majority class. If it only improves overall accuracy without improving \gls{auprc}, balanced accuracy, and F1 for the \emph{Mix required} class, the result is not reliable evidence for useful routing.

\subsection{Data validity}
\label{sec:dataValidity}

Using a public benchmark strengthens data validity because questions and gold answers are externally defined rather than constructed to fit the router. That said, 2WikiMultihopQA is not Ericsson documentation, and the conclusions must be scoped to the benchmark setting unless an Ericsson extension is later added.
...
The primary analysis evaluates held-out label prediction. Before choosing a threshold, \gls{auprc} is used to measure how well the router ranks \emph{Mix required} queries above \emph{Naive sufficient} queries under class imbalance \cite{saito2015precision}. For thresholded evaluation, the router score \(s_\phi(q_i)\) is converted into a class prediction with threshold \(\tau\).
...
The software environment described in Section~\ref{sec:hardwareSoftware} supports all data analysis. The classifier evaluation compares the ModernBERT router with a \gls{tfidf} logistic-regression baseline and a majority-class baseline, reporting both threshold-independent metrics (\gls{auprc}) and thresholded metrics (precision, recall, F1, balanced accuracy). Offline policy analysis combines saved router predictions with the measured execution times for both retrieval modes.

All dataset splits, model predictions, metric calculations, and policy comparisons are saved so that reported tables and figures can be regenerated from the same inputs.


\section{Evaluation framework}
\label{sec:evaluationFramework}

Evaluation has two sides. Classifier evaluation asks whether the ModernBERT router predicts held-out judged labels better than a \gls{tfidf} logistic-regression baseline and a majority-class baseline, using \gls{auprc} for ranking quality under class imbalance and thresholded metrics for concrete routing decisions. Routing-policy evaluation asks whether a thresholded router creates useful operating points between always-\emph{Naive} and always-\emph{Mix}. Both perspectives matter: a router with good classification metrics may still be unattractive if it routes too many examples to the expensive mode, or misses too many examples where \emph{Mix} is required.

Threshold selection uses validation data only. Test data is reserved for final reporting of the selected threshold and threshold curves, reducing the risk of overfitting to the test set.

\section{System documentation}
\label{sec:systemDocumentation}

The primary documentation is a maintained project repository with a clear README covering the purpose of the prototype, the required environment, the main pipeline steps, reproduction instructions, and output organization. For a software-based thesis this is the most useful form because it stays close to the code and can be updated as the implementation changes.

The report itself documents the system architecture, the data flow from benchmark sample to routing label, the evaluation procedure, and the main configuration choices. Detailed operational material can be placed in an appendix if needed.

The guiding standard is traceability. Any reported result should be traceable back to the dataset split, retrieval outputs, judge labels, router predictions, and evaluation procedure that produced it. Since the prototype is an experimental research system rather than a deployed service, this matters more than a separate operations manual.
